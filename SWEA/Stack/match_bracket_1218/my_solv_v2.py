...
for tc in range(1, T+1):  # 테스트 케이스만큼 반복
    ...
    for i in brackets:  # 문자열만큼 반복
        if i in right_bracket.keys():  # 현재 문자가 여는 괄호라면
            top += 1  # 처음 요소를 1씩 더하고
            stack[top] = i  # 스택의 top 위치를 현재 문자로 갱신

        elif i in right_bracket.values():  # 현재 문자가 닫는 괄호라면
            if top == -1:  # 처음요소가 제일 아래라면
                ans = 0  # 유효하지 않다고 두고
                break  # 즉시 반복 종료

            # 옳은 괄호 조합에서의 스택의 top 위치 문자가 현재 문자와 다르다면
            elif right_bracket[stack[top]] != i:
                ans = 0  # 유효하지 않다고 두고
                break  # 즉시 반복 종료
